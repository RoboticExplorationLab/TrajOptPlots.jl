...
visualize!(vis, model, [1,2,0,0.])

delete!(vis)
model = RobotZoo.Pendulum()
TrajOptPlots.set_mesh!(vis, model)
visualize!(vis, model, [pi/2,0])

delete!(vis)
model = RobotZoo.Cartpole()
TrajOptPlots.set_mesh!(vis, model)
visualize!(vis, model, [0.5, pi/2, 0,0])

delete!(vis)
model = RobotZoo.Acrobot()
TrajOptPlots.set_mesh!(vis, model)
visualize!(vis, model, [pi/4, pi/4, 0,0])

delete!(vis)
model = RobotZoo.DubinsCar()
TrajOptPlots.set_mesh!(vis, model)
visualize!(vis, model, [0.5, 0, pi/4])

delete!(vis)
model = RobotZoo.Quadrotor{MRP}()
TrajOptPlots.set_mesh!(vis, model)
g = MRP(expm(SA[1,0,0]*pi/4))
visualize!(vis, model, [0,0,1, g.x, g.y, g.z, 0,0,0, 0,0,0])
...
visualize!(vis, model, [0,0,0, Rotations.params(q)...])
